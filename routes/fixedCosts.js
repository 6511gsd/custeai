...
router.get('/', requireSubscription, async (req, res) => {
  try {
    const { rows } = await query(`
      SELECT * FROM fixed_costs WHERE company_id = $1 AND is_active = true ORDER BY category, name
    `, [req.companyId]);
...
router.post('/', requireSubscription, async (req, res) => {
  try {
    ...
    `, [req.companyId, name, category || 'outros', amount, due_day || null, is_recurring !== false, notes]);
